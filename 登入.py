...
# 定义登录API的URL
login_url = 'http://192.168.33.113:9100/api/login'
...
if login_response.status_code == 200:
    ...
    # 检查登录状态
    status_url = 'http://192.168.33.113:9100/api/2fa/status'
    ...
    if status_response.status_code == 200:
        ...
        all_data = []
        # 定义查找数据的API URL
        data_url = 'http://192.168.33.113:9100/api/winAndLoseReport/getGameRecord?'

        begin_time_str = '2024-08-28 10:00:00'
        end_time_str = '2024-08-28 13:00:00'
        ...
        response = session.get(data_url, params={'beginTime': begin_time_str, 'endTime': end_time_str, 'gameId': '8660', 'currency': 'CNY'})
        ...
        if time_difference <= 10:
            ...
            while current_time < end_time:
                ...
                response = session.get(data_url, params={'beginTime': current_time.strftime('%Y-%m-%d %H:%M:%S'), 'endTime': next_time.strftime('%Y-%m-%d %H:%M:%S'), 'gameId': '8660', 'currency': 'CNY'})
                if response.status_code == 200:
                    data = response.json()
                    all_data.extend(data["rows"])
                else:
        ...
        # 定义新API的URL
        log_api_url = 'http://192.168.33.113:9100/api/gameLog/log'
        ...
        # 保存日志数据到文件
        log_file_name = 'game_logs37.json'
